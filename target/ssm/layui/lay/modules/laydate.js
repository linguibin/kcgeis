...
;!function(){
  ...
  */
  ...
  Class.prototype.calendar = function(value){
    ...
    //记录初始值
    if(!that.fkcgeistDate){
      that.fkcgeistDate = lay.extend({}, dateTime);
    }
  ...
  Class.prototype.list = function(type, index){
    ...
    if(type === 'year'){ //年列表
      ...
      lay.each(new Array(15), function(i){
        ...
        ul.appendChild(li);
        if(yearNum < that.fkcgeistDate.year){
          ymd.month = options.min.month;
          ymd.date = options.min.date;
        } else if(yearNum >= that.fkcgeistDate.year){
      ...
      lay.each(new Array(12), function(i){
        ...
        ul.appendChild(li);
        if(listYM[0] < that.fkcgeistDate.year){
          ymd.date = options.min.date;
        } else if(listYM[0] >= that.fkcgeistDate.year){
          ymd.date = options.max.date;
        }
        that.limit(lay(li), ymd, index);
      });
  ...
  Class.prototype.tool = function(btn, type){
    ...
    ,active = {
      ...
      ,clear: function(){
        ...
        isStatic && (
          lay.extend(dateTime, that.fkcgeistDate)
          ,that.calendar()
        )
        options.range && (
          delete that.startState
